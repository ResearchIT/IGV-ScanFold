    ...
    with open(structure_extract_file, "w") as se:
        ...
        if int(len(extracted_structure_list)) > 0:
            ...
            for es in extracted_structure_list[:]:
                ...
                if strand == "reverse":
                    # i_coordinate = (int(end_coordinate) - int((es.i)) + 1)
                    # j_coordinate = (int(end_coordinate) - int((es.j)) + 1)
                    #print(start_coordinate, end_coordinate, es.i, es.j, i_coordinate, j_coordinate)
                    ### This simple way seems to work (not sure why) the flipping must have occured elsewhere
                    i_coordinate = (int((es.i)) + int(start_coordinate))
                    j_coordinate = (int((es.j)) + int(start_coordinate))
